...
<center>**A generalised framework for a climate change vulnerability analysis of a system**</center>
...
<center>![sysmod](processFlow.png)</center>
<center>**Overview of the functions and arguments in this package. The main package functions and arguments are shown in blue, and orange boxes indicate modules that can change with each case study. Default options for these are included in the package, including a demo system model and climate data**</center>
...
data(tank)    #load data -this contains obs for this example
...
<center>**Schematic of the system model**</center>
...
systemArgs<-list(roofArea=50,                # roof area in m2
                 ...
                 write.file=T,               # write output tank timeseries to file T/F?
                 fnam="tankperformance.csv", # name of file
                 metric="reliability")       # performance metric chosen for reporting

performanceOut=tankWrapper(data=obs,systemArgs=systemArgs)  
...
<center>![process](process.png)</center>
<center>**Schematic of the system model**</center>
...
Once you are comfortable producing performance spaces it is possible to evaluate and compare different system configurations. For example, it is possible to compare different tank sizes for the same property, or compare different contributing roof areas feeding into the same size tank. The figure below illustrates comparison of two performance spaces (contributing roof area of 100 and 110 m2) to show the impact that roof area has on the reliability of the tank across a range of potential climates. A set of climate projections for the region has been overlain in black. Comparing the contour lines of the reliability performance metric, it is clear that the 10% increase in the contributing roof area increases reliability across the space. Comparing the orientation of the contours, it is clear that the each system (100m2 and 110m2) responds to changes in climate in a similar way (e.g. the system is still more sensitive to changes in annual total rainfall than annual average temperature).

```{r simple,echo=FALSE, dpi=300, results='hide',warning=FALSE,message=FALSE}
data(tank)
data(climdata2030)
...
plotArgs<-list(title="Roof area 100m2",performancelimits=c(0.705, 0.78),ylim=c(0.8,1.201))

p1<-quickSpace(obs=obs,
...
```{r simple2,echo=FALSE,dpi=300, results='hide',warning=FALSE,message=FALSE}
...
plotArgs<-list(title="Roof area 110m2",performancelimits=c(0.705, 0.78),ylim=c(0.8,1.201))

p2<-quickSpace(obs=obs,
           attPerturb=attPerturb,
           exSpArgs=exSpArgs,
           systemModel = tankWrapper,
           systemArgs = systemArgs,
...
```{r simple3, results='hide',fig.align='center',echo=FALSE,dpi=300, fig.width=14,fig.height=7}
...
<center>![quickSpace](quickSpace.png)</center>
<center>**An overview of the quickSpace function, and how it implements the three main package steps**</center>
...
```{r quickSpace, results='hide',fig.align='center',dpi=300, fig.width=7,fig.height=7}
data(tank)
data(climdata2030)
...
plotArgs$title="Roof area 100m2"
...
<center>![type](exSpArgs.png)</center>
<center>**Layouts of the exposure spaces created by 'oat' and 'regGrid'. oat will perturb each attribute separately, only sampling along the axes. regGrid will do a complete enumeration of the samples along each axis.**</center>
...
<center>![](SimpleScale.png)</center>
<center>**Example of two scenarios created using simple scaling.**</center>
...
Consider an exposure space generated for both temperature and precipitation. For this test you wish to vary temperature from -1 degrees to 4 degrees in 1 degree increments, and precipitation from 90% of historical average through to 150% of historical average in 10% increments, as illustrated below.
...
This function call generates a significant amount of output, storing the timeseries for each scenario in .csv and .Rdata formats. After a simulation is completed, a folder will appear in your working directory with the title being the *SimDirectory* argument. If you are unsure, type getwd() to determine the working directory.
...
<center>![](Stochastic.png)</center>
<center>**An example of a stochastic scenario produced to match historical conditions**</center>
...
- PET_ann_rng_m - the range between the 5th percentile PET and the 95th percentile PET (mm)
- PET_ann_90pX_m - the percentage of days with PET amounts above a user defined threshold (%)
...
<center>![](attSel.png)</center>
<center>**A demonstration of how five different attributes are calculated on a stochastic time series**</center>
...
<center>![](diog1.png)</center>
<center>**'Traffic light' plots showing how well each requested attribute was simulated compared to the target. In this example, most attributes were simualted well, but 99th percentile rainfall was not, as it is 21% off target.**</center>
...
<center>![](diog2.png)</center>
<center>**An example diagnostic plot showing a comparison between observed data, and monthly averaged simulated data**</center>
...
# this is the call to the actual system function: tankPerformance
...
# out<-scenarioGenerator(obs=obs,                           #Function call from section 4
#                        modelTag = modelTag,
#                        attPerturb=attPerturb,
#                        exSpArgs = exSpArgs)
...
plot<-performanceSpaces(data=out,                            #Results of the scenarioGenerator function
                  plotTag="Heat",
                  systemModel = tankWrapper,
                  systemArgs = systemArgs)
...
```{r plotheat2, fig.align='center',dpi=300, fig.width=7,fig.height=7}
...
```{r plotheat3, fig.align='center',dpi=300, fig.width=7,fig.height=7}
...
```{r plotcontour, fig.align='center',dpi=300, fig.width=7,fig.height=7}
...
```{r plotcontour2, fig.align='center',dpi=300, fig.width=7,fig.height=7}
...
```{r plotoat2, fig.align='center',dpi=300, fig.width=21,fig.height=7,warning=FALSE,message=FALSE}
...
```{r  testclim,fig.align='center',dpi=300, fig.width=7,fig.height=7,warning=FALSE,message=FALSE}
data(tankclim)
data(climdata2030)          #loading climate data, can also use 2050, 2070 and 2085

# From a previous example
# tank.plot<-performanceSpaces(data=out,
#                   plotTag = "Heat",
#                   plotArgs=plotArgs,
#                   systemModel = tankWrapper,
#                   systemArgs = systemArgs)
...
```{r  testclim2,fig.align='center',dpi=300, fig.width=7,fig.height=7,warning=FALSE,message=FALSE}
